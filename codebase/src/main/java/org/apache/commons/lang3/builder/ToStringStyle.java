...
public abstract class ToStringStyle implements Serializable {
    ...
    static void register(final Object value) {
        if (value != null) {
            final Map<Object, Object> m = getRegistry();
            if (m == null) {
                REGISTRY.set(new IdentityHashMap<Object, Object>());
            }
    ...
    protected void appendDetail(final StringBuffer buffer, final String fieldName, final Collection<?> coll) {
        // Try to snapshot the collection to avoid ConcurrentModificationException
        // if the collection is modified concurrently while building the string.
        // Fall back gracefully if snapshotting/iteration fails under concurrent
        // modification (avoid throwing ArrayIndexOutOfBoundsException).
        buffer.append('[');
        try {
            ...
            // toArray() or the subsequent iteration can fail for some
            // concurrent implementations (LinkedList.toArray may hit AIOOBE)
            // Try a defensive iteration; if that also fails, fall back to
            // a compact stable representation that will not throw.
            try {
                boolean first = true;
                ...
                // Last resort: append a stable, non-iterating summary.
                buffer.append(coll.getClass().getName());
                buffer.append('@');
                buffer.append(Integer.toHexString(System.identityHashCode(coll)));
                buffer.append(" size=");
                try {
    ...
    protected void appendDetail(final StringBuffer buffer, final String fieldName, final Map<?, ?> map) {
        // Try to snapshot the map entry set to avoid ConcurrentModificationException
        // when the map is modified concurrently. Fall back gracefully if
        // snapshotting/iteration fails under concurrent modification.
        buffer.append('{');
        try {
            ...
            try {
                ...
                buffer.append('@');
                buffer.append(Integer.toHexString(System.identityHashCode(map)));
                buffer.append(" size=");
                try {
